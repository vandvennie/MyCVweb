...
function getReward() {
  ...
  if (checkedBoxes >= 12) {
    ...
    setTimeout(() => {
      ...
      // Scroll to the bottom of the banner
      window.scrollTo({
        top: banner.offsetTop + banner.offsetHeight,
        behavior: "smooth"
      });
    }, 500); // Delay to ensure the image is loaded
  } else {
...
function updateState() {
  ...
  getReward();// Update reward pic when checkbox state changes
}
...
function restoreState() {
  ...
  getReward();// Update reward pic when restoring state
}
